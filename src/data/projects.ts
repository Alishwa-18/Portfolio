...
export const projects: Project[] = [
  ...
  },
  {
    ...
    approach:
      "Built a claims agent where the LLM decides when to call a tool, but the actual risk decision is deterministic Python, not a model guess: assess_risk_score runs a weighted rule engine (claim amount vs. a $25k high-value threshold, claim velocity of 2+ claims in 30 days, inactive policy status, and a fraud-keyword scan for phrases like 'cash only' or 'no police report'), producing a 0–1 score and a low/elevated/severe band. route_claim then assigns the claim to auto-approval, adjuster, or senior-adjuster queues with an SLA (72h / 24h / 4h) based on that band.",
    result:
      "Every triage decision traces back to an explicit, listed reason, such as 'claim amount exceeds high-value threshold' or 'one prior claim in the last 30 days', instead of an opaque model judgment call.",
  ...
  },
  {
    ...
    result:
      "An assistant that actually does the thing instead of describing how you'd do it: it checks your calendar, files a Notion task, and remembers the conversation next time you open it.",
  ...
  // ---------------------------------------------------------------------
  {
    ...
    problem:
      "Remote teams switch apps constantly just to log a task, and that friction means the task often doesn't get logged at all.",
    approach:
  ...
  },
  {
    ...
    category: "n8n Automation",
    problem:
      "High-volume invoices arriving by email had to be manually read, checked against purchase orders, and paid out, which was slow and error-prone at volume.",
    approach:
  ...
  // ---------------------------------------------------------------------
  {
    ...
    result:
      "Leads move from form submission to a confirmed, paid booking with WhatsApp confirmations at every stage, with no manual invoice creation.",
  ...
  },
  {
    ...
    result:
      "Cross-platform automation experience gained from comparing how the same intake-to-notification pattern is built in n8n, Make, and Zapier.",
